...
Status (2026-09-17): written in response to the audit of commit 6d111fc (item 2: Theorems A and D
were proved in different models). Self-checked; **not reviewed by a domain expert.**
...
- (D5′) g(h) = φ(|h|) ≤ Ḡ, with φ(u) ≤ κ̄u on [0, u_0] and φ(u) > 0 for u > 0.

Under (G), (D3) holds with σ_a² ≡ σ² and E[Y⁴ | A, H] ≤ C(B, σ). So every μ ∈ 𝓜_D satisfies the
hypotheses of Theorem D.

Fix u_0 < u_1 < u_2 with F([u_1, u_2]) > 0. Set γ := φ(u_1) > 0, and

  b(h) = sin²(π(h − u_1)/(u_2 − u_1)) · 1{u_1 ≤ h ≤ u_2}.

Then 0 ≤ b ≤ 1, b is ℓ_b-Lipschitz on ℝ with ℓ_b = π/(u_2 − u_1), E[b(H)] > 0, and b ≡ 0 on
(−∞, u_1], so in particular near the boundary. Set t_max := min{B, L/ℓ_b}. (Since b vanishes on
[−u_0, u_0], L/ℓ_b is not needed for (D2); it is kept so the argument also covers a globally
L-Lipschitz model.)
...
observation with H ∈ [u_1, u_2], where g ≥ γ.

---

## Corollary S

Consider any design in 𝓜_D (adaptive or not), with cumulative exploration loss R_n(μ). Let
N = Σ_i 1{A_i = 0, H_i ∈ [u_1, u_2]}.

**(S1) Boundary side, pointwise.** Take the non-adaptive common-temperature design τ_n = n^{−α},
α ∈ (1/2, 1). Then R_n → 0, and for **every** μ ∈ 𝓜_D the Wald interval of Theorem D for β_0(μ) has
asymptotic coverage 1 − α. This is Theorem D and its corollary. The guarantee is pointwise in μ; no
...
3. *Link to loss.* Each observation counted in N is off-greedy with g(H_i) ≥ γ, so
   γN ≤ Σ_i g(H_i)1{A_i = ā(H_i)}. Taking expectations, E_0[N] ≤ R_n(μ^0)/γ.
...
In one model 𝓜_D, the same logged data from a common temperature τ_n = n^{−α}, α ∈ (1/2, 1):
