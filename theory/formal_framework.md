# Formal Framework (v0.4 — SINGLE SOURCE OF TRUTH)
...
1. **생성형 처치의 estimand 정식화**: 처치를 (전략정책 π, 생성기 G, 정보환경 E)의 결합으로 정의하고, deployment / policy / turn-level excursion 효과를 분해하는 estimand 체계. **(v0.4.1 층위 정리) JCI 논문에서는 일반 적응 생성 행동공간 A_t ∈ 𝒜로 서술**하고(이산 전략·연속 생성 파라미터·form×dose 모두 특수사례), **턴별 2×2(form × dose) factorial은 PA 논문의 실험 구현 객체**로 둔다 — 본 framework의 2×2 표기는 running example이자 PA 트랙 사양.
2. **Temperature-indexed overlap collapse and localization** (수학적 명칭 — "정보 붕괴/국소화"는 해석 층위의 표현으로만 사용): 배치 정책의 결정성(유효온도 τ)이 올라갈수록 (a) 모집단 excursion effect의 semiparametric efficiency bound가 지수적으로 악화되고(Result A — 성립 조건은 §3), (b) 정규화된 overlap 가중 ω_τ = p_τ(1−p_τ)는 정책의 학습된 결정경계로 집중된다(Result B). **주의: 고정 τ에서 추정량의 목표가 경계 효과로 "바뀌는" 것이 아니다 — 경계 효과는 별도 estimand β^ov로 명시 정의한다** (nonidentification_note Result A/B).
3. **G-MRT with generated-treatment fidelity**: 대화 내(within-conversation) 턴별 micro-randomization + 잠긴 evidence bank + gated generation + 배정-ITT·차별 비순응 감사로 적응형 생성 처치 효과를 식별하는 프로토콜. **novelty 문구(축소 확정): "LLM에 MRT 최초 적용"이 아니라 "생성형 정치설득의 대화 내 턴별 micro-randomization + 생성 처치 fidelity + information locking"** — LLM×MRT 결합 자체는 행동변화 메시징(arXiv:2506.07275) 등에 선례 존재.

**명시적 강등 (본 논문의 기여가 아님 — discussion 또는 companion paper)**: generator transport(Γ), GPI 내부표현 결합, 다중 비교 동시추론, epistemically constrained policy learning.
...
**Result A (temperature-indexed collapse of the efficiency bound — 본 기여).** τ>0이면 형식적 positivity·식별은 성립한다. **성립 조건(잠금)**: (A1) 𝓗_δ = {h: |Δ(h)| ≥ δ}가 양의 측도 (δ>0); (A2) 𝓗_δ 위에서 조건부 분산 하한 σ̲² = inf_{h∈𝓗_δ, a} σ_a²(h) > 0; (A3) **비모수/semiparametric 모델 클래스** — outcome regression의 파라메트릭 외삽(경계 밖 층으로의 모형 기반 보간)을 허용하지 않는 클래스에서의 주장이다. 이때 ATE형 excursion의 semiparametric efficiency bound에는 E[σ₁²(H)/p_τ(H) + σ₀²(H)/(1−p_τ(H))]가 들어가므로, 하한은 σ̲²·P(𝓗_δ)·exp(δ/τ) 이상으로 폭발한다. **핵심: 이것은 IPW만의 문제가 아니라 (A3) 클래스의 어떤 정규 추정량으로도 복구되지 않는 정보 한계다** — AIPW로 바꿔도 관측되지 않은 반사실 정보는 생기지 않는다. (파라메트릭 외삽을 믿으면 이 한계를 피할 수 있으나, 그것은 식별을 가정으로 사는 것이다 — 논문에 이 한 문장을 명시.)
...
**Result B (overlap collapse and localization — 본 기여).** ω_τ(H) = p_τ(H){1−p_τ(H)}로 정의된 overlap 가중은, Δ(H)가 연속이고 0 근방에 밀도를 가지면 τ↓0에서 결정면 {Δ(H)=0}으로 집중된다. 따라서 **안정적으로 남는 overlap 질량은 경계에 국소화**되며(해석: 그곳의 인과정보만 안정적으로 관측 가능), 이를 별도 estimand β^ov_τ = E[ω_τ(Y(1)−Y(0))]/E[ω_τ]로 명시 정의한다 (overlap weighting/empirical equipoise 문헌과 연결).
...
3. G^lock 구현 충실도: pilot 전 sandbox 테스트 (platform/design_spec.md).
