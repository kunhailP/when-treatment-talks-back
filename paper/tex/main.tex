...
Second, even with formal positivity, the semiparametric efficiency bound for
population excursion effects degrades exponentially in the policy's effective
inverse temperature: no regular estimator in the nonparametric class escapes
the rate (Result~A); under a uniform-recurrence condition the bound compounds
geometrically across turns (Proposition~A-T). Third, the overlap weight mass
concentrates on the policy's learned decision boundary as temperature falls,
so the causal information that remains stably observable is localized there;
the boundary effect is a distinct --- and policy-relevant --- estimand
$\beta^{\ov}$, not a silent replacement for the population effect (Result~B,
proved in one dimension). A phase diagram in $(n,\tau)$ with an exact boundary
--- $\log\E[1/p_\tau]=\log(1+\tau\sinh(1/\tau))$ for our main design ---
separates estimable, variance-explosion, and deceptively stable-but-biased
regimes. Re-querying three RLHF-tuned open-weights models on 200 real debate
contexts shows median per-context strategy entropy of exactly zero and
modal-strategy shares of 94.5--98.6\%, implying $\geq 10^{5}$ conversations
for any minority-strategy contrast. An audit of 56{,}831 deployed persuasion
conversations shows that randomized prompt instructions explain only
12--32\% of realized evidence-dose variance. We propose the generative
...
policy attitudes with durable effects \citep{hackenburg2025levers}. Yet the
meta-analytic picture of political persuasion is one of small and
heterogeneous effects ($g \approx 0.02$, $I^{2}\approx 76\%$), which makes
decomposition --- \emph{which turn-level behaviors of the AI cause belief
change} --- the scientifically and politically urgent question. The strongest
...
\citep{shi2025distal} --- using ordinary WCLS for terminal outcomes is a
...
efficiency bound is deferred to the appendix; our contribution is the
...
\E[\omega_\tau\{Y(1)-Y(0)\}]/\E[\omega_\tau]$. Assume $H$ has continuous
density $f$; $\Delta \in C^1$ with isolated simple roots; a tail-separation
condition ($\inf_{|h - r| \geq \epsilon} |\Delta(h)| > 0$ for all
$\epsilon>0$); and continuity of $f$ and of $c(h) = \E[Y(1)-Y(0) \mid H=h]$
near the roots, $c$ bounded. Then, as $\tau \downarrow 0$, the normalized
overlap measure converges weakly to a point mass at the decision boundary;
with a unique root at $0$,
\[
...
mass distributes proportionally to $f(r_j)/|\Delta'(r_j)|$. The multivariate
coarea version remains at appendix/conjecture status.
\end{proof}
...
estimators \citep{shi2025distal}, not terminal-outcome WCLS. Policy values:
...
H\'ajek shows bias 0.46 with SD 0.30 --- biased-yet-stable; oracle AIPW does
not rescue coverage (Result~A is not an IPW artifact); the oracle
