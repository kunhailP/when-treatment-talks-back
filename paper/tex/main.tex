...
\begin{abstract}
\noindent Conversational AI systems choose what to say turn by turn, adapting
to the person they are talking to. Their deployment logs are the data most
analysts will have, and the question those logs invite is which of the
system's behaviors caused the belief change. We show that adaptivity itself
obstructs that question, and we quantify the obstruction.

Treating the delivered treatment as a triple --- a strategy policy $\pi$, a
generation kernel $G$, and an information environment $E$ --- we derive the
efficient influence function for the turn-level excursion effect of such a
system. Its semiparametric efficiency bound grows exponentially in a
\emph{policy decisiveness} index $D(h) = -\log\min_a \pi(a \mid h)$, a
functional of the behavior policy alone that is identified from logs and of
which softmax temperature is one special case; the classical
average-treatment-effect bound is the degenerate case of ours. This is more
than a large constant. Along policy sequences whose decisiveness grows with
sample size, a two-point argument bounds the minimax risk away from zero:
no estimator recovers turn-level effects at any rate, and the regime in
which estimates look stable while remaining wrong is exactly the regime in
which two policies with different effects are statistically
indistinguishable.

What information survives is located. As the policy sharpens, the overlap
mass concentrates on the policy's learned decision boundary, and the
estimand that remains estimable is a boundary effect --- formally a
regression-discontinuity quantity, at a threshold the policy chose rather
than the analyst. We prove this in one dimension and, by a coarea argument,
on a decision manifold in $\mathbb{R}^d$. Re-querying three RLHF-tuned
open-weights models on real debate contexts suggests deployed policies are
sharp enough for this to bind, though we show the measurement is a property
of a label projection and report what it cannot establish. The design
consequence is immediate: turn-level causal knowledge about conversational
AI has to be randomized into the conversation, not recovered from it, and we
give the protocol that does so.
\end{abstract}
...
policy attitudes with durable effects \citep{hackenburg2025levers}. Measured
against the baseline of political persuasion research, where meta-analytic
average effects sit well under one percentage point
\citep{kalla2018minimal}, these are not small findings.

They are also not the findings anyone wants. Knowing that a system persuades
is the beginning of the question. The behavior we would regulate, replicate,
or defend against is not ``conversational AI'' but something narrower: the
system asserted rather than asked, or supplied five facts rather than one, at
this point in this conversation. The strongest lever in
\citet{hackenburg2025levers} --- a reward model selecting responses turn by
turn --- is precisely a turn-level adaptive policy, and its logs are what an
analyst inherits.

The natural move is to treat those logs as observational data on a
time-varying treatment and apply the corresponding machinery. This paper is
about why that move fails, and it fails for a reason specific to adaptive
systems rather than for the familiar reason of confounding. A policy that
has learned which action is better at a given history stops taking the other
one. The counterfactual contrast an analyst wants to estimate is exactly the
contrast the system has learned not to generate. Competence at the task and
informativeness about the task are in tension, and the tension is
quantifiable.

Three things follow. Where the learned policy is history-deterministic, the
turn-level effect is not identified at all --- a support failure, standard
since \citet{robins1986new}, whose interest here is that RLHF preference
optimization and low-temperature deployment make the triggering condition
systematic rather than accidental. Where positivity survives, it survives
uselessly: the efficiency bound for the turn-level effect grows exponentially
in how decisive the policy is, and along sequences of increasingly decisive
policies the minimax risk does not go to zero at any sample size. And the
information that does survive is not a diminished version of the population
effect but a different estimand: it lives on the policy's decision boundary,
where the policy is undecided, which in a confident deployment is a
vanishing minority of traffic.

That last point is where the negative result turns useful. A boundary
estimand at a learned threshold is a regression-discontinuity quantity, with
the interpretive apparatus that comes with it --- including the knowledge
that it does not describe the population the deployment actually served.
Analysts who report an effect from adaptive logs are, at best, reporting
this, and should say so.

Since the obstruction is created by the deployed policy before the analyst
sees anything, no estimator removes it; only a different design does. The
final section develops the generative micro-randomized trial: randomize the
action within the conversation, lock the evidence available to the
generator, and treat the generate-and-check procedure as the treatment
kernel, so that the quantity identified is assignment to a generative
protocol rather than the effect of a realized sentence. The experiment
itself is a companion paper; what belongs here is the argument that it is
necessary.

\paragraph{Relation to existing literatures.} Three are adjacent.
...
established rather than assumed --- under a separability condition, and the
authors warn that adjusting naively on representations destroys it. Our
object differs in where overlap is lost: there it is a property of the
representation the analyst conditions on, here it is destroyed by the
deployed policy's own decisiveness before analysis begins. Second, the
estimation-side face of the problem is studied as off-policy evaluation
under deficient support \citep{sachdeva2020off,dudik2011doubly,
thomas2016data,jiang2016doubly,kallus2020double}. That literature bounds
particular estimators; our statements are about the efficiency bound and the
minimax risk, which closes the ``use a better estimator'' response by
construction, and its remedies --- truncation, regularized weights, support
restriction --- change the estimand without characterizing what they change
it to, which is what Section~\ref{sec:resultB} supplies. Third, the
econometrics of limited overlap \citep{khan2010irregular,damour2021overlap,
li2018balancing} is the closest antecedent for the shape of our results; the
contribution here is to index the failure by a policy functional that a
deployment actually determines, and to show what the surviving estimand is.
Micro-randomized trials \citep{qian2022microrandomized,boruvka2018assessing}
supply the excursion estimand we adopt, and stochastic-intervention theory
...
between assignment and realization, and the fidelity problem that comes with
it.
