...
even when positivity formally holds, derived from the efficient
...
and, under an additional recurrence condition, compounding across turns;
and (iii) localization --- the overlap mass that survives
...
effect and formally a regression-discontinuity quantity. We verify each mechanism in simulation, locate
...
Contributions, three: (1) an estimand framework for generative treatments
with a general adaptive action space, and the efficient influence function
for its turn-level excursion estimand (Proposition~\ref{prop:eif});
(2) collapse and localization indexed by a \emph{policy decisiveness}
functional that is identified from logs --- an efficiency bound growing
exponentially in it (Proposition~\ref{prop:a}), a matching minimax
impossibility along drifting policy sequences
(Proposition~\ref{prop:minimax}), and localization of the surviving overlap
mass onto the decision boundary in one dimension
(Proposition~\ref{prop:b}) and on a manifold in $\mathbb{R}^d$
(Corollary~\ref{cor:manifold}); (3) the
...
variance or regret; Propositions~\ref{prop:a} and \ref{prop:minimax} are
statements about the semiparametric efficiency bound and about the minimax
risk, which closes the ``use a better estimator'' escape by construction.
Second, we index the collapse by a decisiveness functional of the policy
that is identified from logs, and obtain a closed form for it in our main
design. Third, localization
(Proposition~\ref{prop:b}) --- \emph{where} the surviving information lives, and the
...
auditing of generated treatments. We make no claim to be first to combine
language models with micro-randomization; our novelty claim is confined to
...
assign the label, realize from $G$. We derive the efficient influence
function for the factorial excursion contrast in that framework
(Proposition~\ref{prop:eif}) rather than importing a bound stated for the
average treatment effect, and our substantive addition is the decisiveness
collapse that follows from it.

\paragraph{Overlap weighting.} $\beta^{\ov}$ connects to overlap weights and
empirical equipoise \citep{li2018balancing}; Proposition~\ref{prop:b} and
Corollary~\ref{cor:manifold} give the sharp-policy limit of that estimand,
which is a boundary effect in the regression-discontinuity sense
\citep{hahn2001identification,keele2015geographic} at a threshold the policy
learned rather than one the analyst chose.

\paragraph{Information equivalence.} \citet{dafoe2018information} are a
caution rather than a licence: they argue that manipulations of one
attribute routinely carry information about others, and are pessimistic
about isolating a focal dimension. We take the warning literally --- it is
why we do not call dose ``information'' but a high-information communication
package, and why the fidelity audit measures what the instruction actually
changed rather than assuming it changed only one thing.
