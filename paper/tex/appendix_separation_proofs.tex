...
Write $e=e_\tau(H)=\sgm(H/\tau)=\Prob(A=1\mid H)$ and $\omega=e(1-e)=K(H/\tau)$ with $K(u)=\exp(u)/\{1+\exp(u)\}^2$,
and $m_\tau=\E\omega$. We use
\begin{gather*}
\int K=1,\qquad \int|u|K(u)\,du=2\log2,\qquad \int K(u)\sgm(u)\,du=\tfrac12,\\
K(u)\le \exp(-|u|),\qquad \int_0^\infty v\,\sgm(-v)\,dv=\tfrac{\pi^2}{12}.
\end{gather*}
...
with dominating function $\|\varrho\|_\infty\bar fK(u)$,
\[
\E[K(H/\tau)^j(1-e)^r\,e^s\,\varrho(H)]=\tau f(0)\varrho(0)\int K^j\sgm(-u)^r\sgm(u)^s\,du+o(\tau).
\]
...
$\E[A(1-e)^2\mid H]=\omega(1-e)$,
\[
s_n^2:=\Var\xi=m_\tau^{-2}\E\big[\omega\{(1-e)(\sigma_1^2+(\mu_1-\bar\mu_1)^2)+e(\sigma_0^2+(\mu_0-\bar\mu_0)^2)\}\big].
\]
...
$q_\tau(h)=\sgm(-|h|/\tau)$, which equals $e_\tau(h)$ for $h<0$ and $1-e_\tau(h)$ for $h>0$. On $|h|\le u_0$,
...
$n^{-1}(\sigma^2+B^2)\E[1/e+1/(1-e)]=n^{-1}(\sigma^2+B^2)\E[1/q+1/(1-q)]\le n^{-1}(\sigma^2+B^2)(\E[1/q]+2)$,
since $\{e,1-e\}=\{q,1-q\}$ and $q\le1/2$. Pointwise $1/q=\max\{2,\sqrt g/c_n\}\le\sqrt g/c_n+2$,
and the choice of $c_n$ gives $\E[1/q]+2\le n\delta_n^2/(\sigma^2+B^2)$. The
loss is $n\E[gq]\le nc_n\E\sqrt g$. For uniform mixing with
$C=\sigma^2+B^2$ and $q=C/(n\delta_n^2-2C)\le1/2$, the same variance bound
...
$S=\{|\Delta|\ge D_{\max}-\eta',a^*=j\}$, with $j$ attaining $\nu=\nu_{\eta'}$. There
$q_\tau\le \exp(-(D_{\max}-\eta')/\tau)$, so $\bar N\le n\nu \exp(-(D_{\max}-\eta')/\tau)$, and the van
Trees step of Theorem~\ref{thm:info} with $K=1$ gives
$\delta_n^2\ge \nu^2\sigma^2/(n\nu \exp(-(D_{\max}-\eta')/\tau)+\sigma^2\pi^2/B^2)$. Once
$\delta_n^2\le \nu^2B^2/(2\pi^2)$, this forces
$n\nu \exp(-(D_{\max}-\eta')/\tau)\ge \nu^2\sigma^2/(2\delta_n^2)$, that is,
$\tau_n\ge(D_{\max}-\eta')/\log\{2n\delta_n^2/(\nu\sigma^2)\}$, where the logarithm is
positive because $2n\delta_n^2>\nu\sigma^2$. Nothing here requires
$\delta_n\to0$. On the other hand
\[
R_n=n\E[\phi(|\Delta|)\sgm(-|\Delta|/\tau)]
\ge n\kappa f_{\min}\tau^2\int_0^{u_0/\tau}v\sgm(-v)\,dv
\ge n\kappa f_{\min}c_0\min\{\tau,u_0\}^2 .
\]
Combining the two bounds proves the
claim. With finitely many score values, the same argument gives
$\tau\ge\Delta_{\max}/\log(Cn\delta^2)$ for $\delta$ small enough. The
$\Delta_{\min}$ cell, with mass $f$, then contributes at least
\[
n f\phi(\Delta_{\min})\exp(-\Delta_{\min}/\tau)/2 . \qquad\qed
\]

\paragraph{Remark~\ref{rem:mixture}.} Write $u=|H|$, $r(u)=\sgm(-u)$ for the
off-greedy probability at temperature $1$, and
$q_n=w_nr+(1-w_n)\sgm(-nu)$ for the mixture. Since $q_n\ge w_nr$,
$\E[1/q_n]/n\le\delta^2$ for $w_n=\E[1/r]/(n\delta^2)$, and
$n\E[uq_n]\le\E[1/r]\E[ur]/\delta^2+O(1/n)$. With
$w_n=\E[1/r]/\{n\delta_n^2/(\sigma^2+B^2)-2\}$ the Horvitz--Thompson risk is at
most $\delta_n^2$ uniformly over $\M(B,\sigma)$.

\paragraph{Proof of Proposition~\ref{prop:ctx}.} Solving
$\sgm(-|\Delta|/\tau)=q$ gives the formula. If $g(h)=0$, then $q^*(h)=1/2$, which
is implementable by $\tau=\infty$ (and by any $\tau$ when $\Delta(h)=0$). If
$\Delta(h)\neq0$ and $g(h)>0$, then $q^*(h)=c_n/\sqrt{g(h)}<1/2$ once
$c_n<\sqrt{g(h)}/2$, and
$\log\{(1-q^*)/q^*\}=\log(1/c_n)+\tfrac12\log g(h)+\log(1-q^*)$ with
$c_n\to0$, which gives the limit. \qed

\paragraph{Proof of Lemma~\ref{lem:robust}.} With the cap slack, let
$q=c/\sqrt{\hat g}$ and $\E[1/q]=T$. The loss is $n\E[\sqrt{\hat g}]\E[g/\sqrt{\hat g}]/T$, against
