...
designs: attaining mean squared error $\delta^2$ requires loss at least of
order $(\E\sqrt g)^2/\delta^2$, where $g$ is the operational value gap; this
order is attainable and does not decrease with deployment scale. At a fixed
precision $\delta$, a single deterministic common temperature requires loss at
least of order $n/\log^2(n\delta^2)$, whereas uniform mixing,
context-dependent temperatures, or suitable temperature mixtures attain the
same precision with loss bounded in $n$. As $\delta\to0$, every design's loss
diverges. The results say which question adaptive logs can answer, and what
...
\item \emph{What this means for temperature.} At a given precision $\delta$, a
single deterministic common temperature requires exploration loss at least of
order $n/\log^2(n\delta^2)$ (Theorem~\ref{thm:common}), against a loss of
order $\delta^{-2}$, bounded in $n$, for the designs below. Context-dependent
...
than top-$k$ logging in simulation; \citet{koppel2025explore} extend
...
non-adaptive designs $R_n=n\E[g(H)q(H)]$.
...
$\omega_\tau=e_\tau(1-e_\tau)$ \citep{li2018balancing} is a third,
...
With $e=e_{\tau_n}$, $w_1=A(1-e)$ and $w_0=(1-A)e$, let
...
$q\equiv(\sigma^2+B^2)/(n\delta_n^2)$ gives
...
\[
R_n\ \ge\ \kappa f_{\min}c_0\,n\min\Big\{u_0^2,\ \frac{(G-\eta')^2}{\log^2\{2n\delta_n^2/(\nu_{\eta'}\sigma^2)\}}\Big\},
\qquad c_0=\int_0^1v\,\sgm(-v)\,dv .
\]
\end{theorem}

This is a lower bound. Relative to the loss attainable by
Proposition~\ref{prop:achieve}, it is larger by a factor of order at least
$n\delta_n^2/\log^2(n\delta_n^2)$, which diverges whenever
$n\delta_n^2\to\infty$; at a fixed $\delta$ the attainable loss is bounded in
$n$ while the common-temperature loss is not. With finitely many score values
...
The optimal temperature is cold at the boundary and hot where the policy is
confident.
\end{proposition}

\begin{lemma}[Misspecified gaps]\label{lem:robust}
Compare the designs $q=c/\sqrt{\hat g}$ and $q=c'/\sqrt g$, with $c,c'$ chosen
so that both have the same design variance $\sigma^2\E[1/q]/n$, and assume
the cap is slack, that is, both designs are at most $1/2$ on $\{g>0\}$. Let
...
When $g$ approaches zero near the boundary, the cap binds on a set whose
$F$-mass vanishes as $c\to0$, and the lemma describes the limit.
...
$q\propto g^{-1/2}$ produces identical rows and is not listed separately. In a
