...
\title{Boundary and population effects under sharpening assignment policies}
\author{Kun Woo Park\thanks{\textbf{[TO CONFIRM: department]}, Kookmin University,
...
and their logs are the evidence analysts inherit. We study what logs from a
known, sharpening strategy-assignment policy reveal about the effect of the
assigned strategy, with the text generator held fixed, and measure the cost of
learning by cumulative exploration loss: the operational value forgone when
the logged strategy differs from the one the deployer prefers. In a
one-dimensional score model we prove a separation inside one outcome model.
Under a single common temperature that shrinks with the sample size, the
effect at the policy's decision boundary admits asymptotically valid Wald
inference while exploration loss tends to zero. In the same model, outcome
laws with equal boundary effects but different population average effects
become indistinguishable whenever exploration loss tends to zero, and
uniformly consistent estimation of the population effect requires divergent
worst-case loss. Without extrapolation structure, an information--exploration
inequality for all estimators and adaptive designs shows that uniform
precision $\delta$ requires worst-case loss of order at least
$(\E\sqrt g)^2/\delta^2$, independent of $n$. For small fixed $\delta$, one
deterministic common temperature requires loss of order at least
$n/\log^2(n\delta^2)$, whereas uniform mixing keeps it bounded.
\end{abstract}

\noindent\textbf{Short title:} When the treatment talks back
...
$(\E\sqrt g)^2/\delta^2$, independent of $n$; a design exploring with
...
we do not claim the allocation as new. Cesa-Bianchi et al.~\cite{cesabianchi2017boltzmann} show
...
population targets on that common scale. Khan, Saveski and Ugander~\cite{khan2024off} bound effects
...
$R_n\asymp n\tau_n^2$, because these actions occur ever closer to the boundary,
...
$R_n\le(\sigma^2+B^2)(\E\sqrt g)^2\delta_n^{-2}(1+o(1))$. With
$C=\sigma^2+B^2$ and $n\delta_n^2\ge4C$, uniform mixing
$q\equiv C/(n\delta_n^2-2C)$ also gives
$\sup_{\M(B,\sigma)}\E(\hat\theta-\theta)^2\le\delta_n^2$, with
$R_n=C\,\E[g]\,\delta_n^{-2}(1+o(1))$.
\end{proposition}

The order $\delta^{-2}$ and the functional $(\E\sqrt g)^2$ therefore appear in
both the lower bound and the attainable loss. The constants differ: the lower
...
G-\eta',a^*(H)=j)>0$. Fix $\eta'\in(0,G)$. If an estimator has
$\sup_{\M(B,\sigma)}\E(\hat\theta-\theta)^2\le\delta_n^2$ with
...
When the cap binds, the ratio bound holds in the limit of large design
variance. Suppose $0<a\le\hat g/g\le b<\infty$ on $\{g>0\}$, $\hat g=0$ on
...
$c\to0$. Calibrating $c$ to a common design variance $T=\E[1/q_c]\to\infty$,
...
The designs are matched on design variance, not on realized RMSE. The working
...
guarantee of Proposition~\ref{prop:achieve}. At a fixed $\delta$, the
expected number of off-greedy actions does not grow with $n$, so these Wald
intervals need not approach nominal coverage as $n$ grows. In a
...
No new data were collected or analysed. All results are theoretical or based on
simulations. The simulation code, the result files (with arguments, random
seeds and code commits), plain-text versions of the proofs, and the script that
generates every number and table in Section~\ref{sec:sims} are available at
\url{https://github.com/kunhailP/when-treatment-talks-back}, branch
\texttt{jci-separation}, at the tagged submission version
